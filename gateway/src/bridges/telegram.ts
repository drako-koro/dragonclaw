/**
 * DragonClaw Telegram Bridge
 * Secure Telegram bot integration — acts as a command center
 * Users give orders via Telegram, DragonClaw executes in the VM
 */
...
export class TelegramBridge {
  ...
  private async sendVoice(chatId: number, filePath: string, caption?: string): Promise<boolean> {
    try {
      ...
      const boundary = '----DragonClawVoice' + Date.now();
      const parts: Buffer[] = [];
