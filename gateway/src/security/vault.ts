...
export class Vault {
  ...
  async initialize(): Promise<void> {
    ...
    let passphrase = process.env.DRAGONCLAW_VAULT_KEY || '';
    if (!passphrase) {
      ...
      if (!existsSync(envPath)) {
        ...
        try {
          await writeFile(envPath, `# Auto-generated by DragonClaw on first run\nDRAGONCLAW_VAULT_KEY=${generated}\n`);
          passphrase = generated;
          process.env.DRAGONCLAW_VAULT_KEY = generated;
          ...
          console.warn('     Set DRAGONCLAW_VAULT_KEY environment variable for production use.');
        }
      } else {
        console.warn('  ⚠️  WARNING: DRAGONCLAW_VAULT_KEY not set (check your .env file).');
        console.warn('     Using a random session key. Vault data will NOT persist across restarts.');
      }
    }
