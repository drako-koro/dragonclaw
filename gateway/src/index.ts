...
class DragonClawGateway {
  ...
  async initialize(): Promise<void> {
    console.log('');
    console.log('  ✍️  DragonClaw v5.0.0');
    console.log('  ═══════════════════════════════════');
    ...
    const authorOSCandidates = [
      ...
      join(ROOT_DIR, '..', 'Author OS'),                          // Sibling to DragonClaw project
    ...
    this.projectEngine.setPersonaContextBuilder((personaId) => this.personas.buildPromptContext(personaId));
    ...
    const commandHandlers = this.buildBridgeCommandHandlers('internal');
    this.heartbeat.setAutonomous(
      // Run one project step (reuses the same logic as Discord /project command)
      ...
      // Broadcast status to dashboard and all enabled bridges
      (message: string) => {
        ...
        if (this.discord) {
          this.discord.broadcastToAllowed?.(message);
        }
      },
        ...
        try {
          ...
          if (log.length % 5 === 0 && log.length >= 5) {
            try {
              ...
              const coreLessonsContent = `# DragonClaw Core Lessons\n\n` +
    ...
    if (this.config.get('bridges.telegram.enabled')) {
      ...
      if (token) {
        ...
        this.telegram.setCommandHandlers(this.buildBridgeCommandHandlers('telegram'));
    ...
    if (this.config.get('bridges.discord.enabled')) {
      ...
      if (token) {
        ...
        this.discord.setCommandHandlers(this.buildBridgeCommandHandlers('discord'));
        await this.discord.connect();
        console.log('  ✓ Discord bridge connected (command center mode)');
      } else {
    ...
    this.app.get('*', (_req, res) => {
      ...
      res.sendFile(htmlFile, (err) => {
        if (err) res.status(200).json({ status: 'ok', message: 'DragonClaw running. Dashboard HTML not found.' });
      });
    });
    ...
    await this.activityLog.log({
      type: 'system',
      source: 'internal',
      message: `DragonClaw started — ${providers.length} AI provider(s), ${this.skills.getLoadedCount()} skills`,
      metadata: {
    ...
    console.log('');
    console.log('  ═══════════════════════════════════');
    console.log('  ✍️  DragonClaw is ready to write');
  ...
  async handleMessage(
    ...
    overrideMessages?: Array<{ role: 'user' | 'assistant'; content: string }>
  ): Promise<void> {
    ...
    if (skills.length > 0) {
      this.activityLog.log({
        type: 'skill_matched',
        source: this.getActivitySource(channel),
    ...
    // Project steps normally get a CLEAN message array, but the project engine
    // can inject a curated synthetic history for continuity.
    // Chat messages include conversation history for continuity.
    const messages = overrideMessages && overrideMessages.length > 0
      ? overrideMessages
      : isProjectChannel
  ...
  private resolveInlinePersonaContext(content: string): string {
    ...
    const personaService: any = this.personas as any;
    const findByPenName = typeof personaService.findByPenName === 'function'
      ? (name: string) => personaService.findByPenName(name)
      : null;

    const explicitPatterns = [
      /persona\s*:\s*["“]?([^"\n,;”]+)["”]?/i,
      /using\s+persona\s+["“]?([^"\n,;”]+)["”]?/i,
      /under\s+pen\s+name\s+["“]?([^"\n,;”]+)["”]?/i,
      /write\s+as\s+["“]?([^"\n,;”]+)["”]?/i,
    ];
    ...
    for (const pattern of explicitPatterns) {
      ...
      const persona = findByPenName ? findByPenName(candidate) : undefined;
      if (persona && typeof personaService.buildPromptContext === 'function') {
        return String(personaService.buildPromptContext(persona.id) || '');
      }
    }
  ...
  private buildSystemPrompt(context: {
    ...
    if (context.channel?.startsWith('discord:')) {
      prompt += '# Communication Style (Discord)\n\n';
      prompt += 'You are chatting via Discord. Keep your messages clear, compact, and useful:\n';
      prompt += '- Use short paragraphs and avoid walls of text\n';
      prompt += '- Prefer actionable updates and concise status summaries\n';
      prompt += '- Use bullets only when they improve clarity\n';
      prompt += '- Keep command responses short because they appear in-channel\n\n';
      prompt += 'IMPORTANT — Discord is a COMMAND CENTER, not the manuscript itself:\n';
      prompt += '- Do not dump full chapters into Discord unless explicitly asked\n';
      prompt += '- Treat Discord like mission control for projects, not the writing canvas\n\n';
    } else if (context.channel?.startsWith('telegram:')) {
      prompt += '# Communication Style (Telegram)\n\n';
      prompt += 'You are chatting via Telegram. Keep messages concise, readable on mobile, and command-friendly:\n';
      prompt += '- Use short paragraphs and compact updates\n';
      prompt += '- Prefer short status replies with the next step\n';
      prompt += '- Do not dump long manuscript text unless explicitly asked\n';
      prompt += '- Treat Telegram like a remote command center for projects\n\n';
    ...
    prompt += 'Users can create autonomous projects via Discord, Telegram, or the dashboard.\n';
  ...
  async handleDashboardCommand(input: string): Promise<string> {
    ...
    const workspaceDir = join(ROOT_DIR, 'workspace');
    const handlers = this.buildBridgeCommandHandlers();
    ...
    switch (cmd) {
      ...
      case '/files': {
        ...
        try {
          ...
          // Build numbered file list (like the bridge)
      ...
      case '/clean': {
        try {
          ...
          const subdirs = ['projects', 'exports', 'documents', 'research'];
          let totalFiles = 0;
  ...
  broadcastTelegram(message: string): void {
    if (this.telegram) {
      void this.telegram.broadcastToAllowed(message);
    }
  }
  ...
  isDiscordConnected(): boolean {
    return this.discord !== undefined;
  }

  /**
   * Broadcast a message to all connected Discord channels.
   */
  broadcastDiscord(message: string): void {
    if (this.discord) {
      void this.discord.broadcastToAllowed(message);
    }
  }

  async connectDiscord(): Promise<{ error?: string }> {
    if (this.discord) {
    ...
    const token = await this.vault.get('discord_bot_token');
    if (!token) {
      return { error: 'No discord_bot_token in vault. Save your bot token first.' };
    }

    this.config.set('bridges.discord.enabled', true);

    try {
      this.discord = new DiscordBridge(token, {
        enabled: this.config.get('bridges.discord.enabled', true),
        guildId: this.config.get('bridges.discord.guildId', ''),
      ...
      this.discord.setCommandHandlers(this.buildBridgeCommandHandlers('discord'));
      await this.discord.connect();
      this.audit.log('bridge', 'discord_connected', {});
      this.activityLog.log({
        type: 'system',
        source: 'internal',
        message: 'Discord bridge connected',
      });
      console.log('  ✓ Discord bridge connected (via dashboard, command center mode)');
      return {};
    } catch (error) {
      this.discord = undefined;
      return { error: String(error) };
  ...
  disconnectDiscord(): void {
    if (this.discord) {
      this.discord.disconnect();
      this.discord = undefined;
      this.config.set('bridges.discord.enabled', false);
      this.audit.log('bridge', 'discord_disconnected', {});
      console.log('  ⚠ Discord bridge disconnected (via dashboard)');
    }
  }

  updateDiscordUsers(users: string[]): void {
    if (this.discord) {
      this.discord.updateAllowedUsers(users);
    }
  }

  updateDiscordChannels(channels: string[]): void {
    if (this.discord) {
      this.discord.updateAllowedChannels(channels);
    }
  }

  /**
   * Build command handlers for chat bridges.
   * These let Telegram and Discord commands interact with the project engine,
   * file system, and AI without dumping long content into chat.
   */
  private buildBridgeCommandHandlers(bridgeSource: 'telegram' | 'discord' | 'internal' = 'discord') {
    const gateway = this;
    const workspaceDir = join(ROOT_DIR, 'workspace');
    const activitySource = gateway.getActivitySource(bridgeSource);
    ...
    return {
       ...
       * Returns a short summary for the bridge + accurate word count.
       */
      ...
      > {
        ...
        // Refresh persona prompt context so project steps can write in the assigned voice
        try {
          if ((project as any).personaId && gateway.personas) {
            const personaService: any = gateway.personas as any;
            let personaPromptContext = '';

            if (typeof personaService.buildPromptContext === 'function') {
              personaPromptContext = await Promise.resolve(personaService.buildPromptContext((project as any).personaId));
            } else if (typeof personaService.getPersona === 'function') {
              const persona = await Promise.resolve(personaService.getPersona((project as any).personaId));
              if (persona) {
                const styleMarkers = Array.isArray(persona.styleMarkers) ? persona.styleMarkers.join(', ') : '';
                personaPromptContext =
                  `Pen Name: ${persona.penName || persona.name || 'Unknown'}\n` +
        ...
        const projectMessages = await gateway.projectEngine.buildProjectMessages(project, activeStep, stepUserMessage);
       ...
       * Sends bridge status updates via the callback after each step.
      ...
      async autoRunProject(projectId: string, statusCallback: (msg: string) => Promise<void>): Promise<void> {
        ...
        while (true) {
          ...
          if (gateway.discord?.pauseRequested || gateway.telegram?.pauseRequested || currentProject?.status === 'paused') {
            if (gateway.discord) gateway.discord.pauseRequested = false;
            if (gateway.telegram) gateway.telegram.pauseRequested = false;
          ...
          if (gateway.discord?.pauseRequested || gateway.telegram?.pauseRequested || afterStepProject?.status === 'paused') {
            if (gateway.discord) gateway.discord.pauseRequested = false;
            if (gateway.telegram) gateway.telegram.pauseRequested = false;
  ...
  async shutdown(): Promise<void> {
    ...
    await this.activityLog?.log({
      type: 'system',
      source: 'internal',
      message: 'DragonClaw shutting down',
    });
    await this.audit?.log('system', 'shutdown', {});
    this.server.close();
    console.log('  ✍️  DragonClaw stopped. Happy writing!\n');
  }
}

// ── Start ──
const gateway = new DragonClawGateway();
...
gateway.start().catch((error) => {
  console.error('Failed to start DragonClaw:', error);
  process.exit(1);
});

export { DragonClawGateway };
