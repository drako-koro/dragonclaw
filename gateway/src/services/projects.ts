...
/**
 * Callback to resolve persona prompt context for a project.
 */
export type PersonaContextBuilderFunc = (personaId: string) => string | Promise<string>;
...
export class ProjectEngine {
  ...
  private personaContextBuilder: PersonaContextBuilderFunc | null = null;
  ...
  setPersonaContextBuilder(builder: PersonaContextBuilderFunc): void {
    this.personaContextBuilder = builder;
  }

  private async ensurePersonaContext(project: Project): Promise<void> {
    if (!project?.personaId || !this.personaContextBuilder) return;

    try {
      const personaPromptContext = await Promise.resolve(this.personaContextBuilder(project.personaId));
...
Description: ${description}`;
  ...
  async buildProjectContext(project: Project, step: ProjectStep): Promise<string> {
    await this.ensurePersonaContext(project);
    ...
    if (typeof step.chapterNumber === 'number') context += `**Chapter**: ${step.chapterNumber}\n`;
    context += `\n`;

    if (project.description) {
      context += `## Project Goal\n\n${this.smartTruncate(project.description, 1200)}\n\n`;
    }
    ...
    if (projectConfigBlock) {
      context += `## Project Configuration\n\n${projectConfigBlock}\n\n`;
    }

    const continuitySummary = this.buildContinuitySummary(project, step);
    ...
    if (project.type === 'novel-pipeline' && step.phase) {
      ...
      const relevantSteps = this.selectRelevantContextSteps(project, step, 6);
      if (relevantSteps.length > 0) {
        context += `## Relevant Prior Work\n\n`;
        for (const cs of relevantSteps) {
          context += `### ${cs.label}\n`;
          context += this.smartTruncate(cs.result || '', 2600) + `\n\n`;
        }
      }
    }
    ...
    switch (step.phase) {
      ...
      case 'writing': {
        ...
        if (writtenChapters.length > 0) {
          const recent = writtenChapters.slice(-3);
          context += `## Recent Chapters (for continuity)\n\n`;
          for (const ch of recent) {
            context += `### ${ch.label}\n${truncate(ch.result!, 3200)}\n\n`;
          }
        }
        break;
      }
  ...
  async buildProjectMessages(
    ...
    if (personaPromptContext) {
      messages.push({
        role: 'user',
        content:
          `Persistent persona instructions for this project:

${personaPromptContext}

` +
          'Apply this persona consistently in all future outputs unless the user explicitly overrides it.',
      });
      messages.push({
        role: 'assistant',
        content: 'Understood. I will keep this persona voice consistent across the project.',
      });
    }

    const priorSteps = this.selectRelevantContextSteps(project, step, 4);

    for (const prior of priorSteps) {
      const promptText = this.smartTruncate(prior.prompt || '', 1200);
      const resultText = this.smartTruncate(prior.result || '', 2600);
      messages.push({
        role: 'user',
        content: `Prior project step: ${prior.label}
...
${promptText}`,
      ...
      messages.push({
        role: 'assistant',
        content: `Result from ${prior.label}:
...
${resultText}`,
  ...
  private selectRelevantContextSteps(project: Project, currentStep: ProjectStep, limit = 5): ProjectStep[] {
    const completed = project.steps.filter(s => s.status === 'completed' && s.result);
    if (completed.length === 0) return [];

    const currentIndex = project.steps.findIndex(s => s.id === currentStep.id);
    const beforeCurrent = completed.filter(s => {
      const idx = project.steps.findIndex(ps => ps.id === s.id);
      return idx >= 0 && (currentIndex < 0 || idx < currentIndex);
    });

    const scored = beforeCurrent.map((s, idx) => {
      let score = idx; // preserve recency
      if (currentStep.phase && s.phase === currentStep.phase) score += 50;
      if (typeof currentStep.chapterNumber === 'number' && s.chapterNumber === currentStep.chapterNumber) score += 80;
      if (
        typeof currentStep.chapterNumber === 'number' &&
        typeof s.chapterNumber === 'number' &&
        Math.abs(s.chapterNumber - currentStep.chapterNumber) === 1
      ) score += 30;
      if (s.taskType === currentStep.taskType) score += 20;
      if ((currentStep.phase === 'writing' || currentStep.phase === 'revision') && s.phase === 'outline') score += 25;
      if ((currentStep.phase === 'writing' || currentStep.phase === 'revision') && s.phase === 'bible') score += 25;
      if ((currentStep.phase === 'writing' || currentStep.phase === 'revision') && s.phase === 'premise') score += 10;
      return { step: s, score };
    });
    ...
    return scored
      ...
      .slice(0, limit)
      .map(item => item.step)
      .sort((a, b) => project.steps.findIndex(s => s.id === a.id) - project.steps.findIndex(s => s.id === b.id));
  }

  private buildProjectConfigBlock(project: Project): string {
  ...
  private buildContinuitySummary(project: Project, step: ProjectStep): string {
    const relevant = this.selectRelevantContextSteps(project, step, 6);
    if (relevant.length === 0) return '';

    let summary = '';
    for (const prior of relevant) {
      const kind = [
        prior.phase ? `phase=${prior.phase}` : '',
        typeof prior.chapterNumber === 'number' ? `chapter=${prior.chapterNumber}` : '',
        prior.taskType ? `task=${prior.taskType}` : '',
      ].filter(Boolean).join(', ');

      summary += `- ${prior.label}${kind ? ` (${kind})` : ''}\n`;
      summary += `  Prompt focus: ${this.singleLine(this.smartTruncate(prior.prompt || '', 240))}\n`;
      summary += `  Outcome: ${this.singleLine(this.smartTruncate(prior.result || '', 320))}\n`;
    }
  ...
  private smartTruncate(text: string, max: number): string {
    ...
    const head = Math.max(Math.floor(max * 0.7), 1);
    const tail = Math.max(max - head - 32, 1);
...
)\s{0,3}(?:#+\s*)?(?:chapter|ch\.?)[\s:_-]*(\d{1,3})/ig)]
        .map(m => Number(m[1]))
...
Output ONLY the full revised chapter prose.`,
  ...
  private enrichWithPriorResults(prompt: string, project: Project): string {
    // Prior step results now flow through buildProjectContext() and buildProjectMessages().
    // Keep the user prompt clean and only add a light reminder when needed.
    if (prompt.includes('see system context') || prompt.includes('Use the project context')) {
      return prompt;
    }

    const lastCompleted = [...project.steps].reverse().find(s => s.status === 'completed' && s.result);
    if (lastCompleted) {
      return `[Use the project context, continuity memory, and prior step results. Build directly on "${lastCompleted.label}".]\n\n${prompt}`;
    }
