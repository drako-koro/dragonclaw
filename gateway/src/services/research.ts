...
export class ResearchGate {
  ...
  async setDomains(domains: string[]): Promise<void> {
    ...
    const data = {
      description: 'Approved domains for DragonClaw research. Add domains as needed for your writing projects.',
      domains: sorted,
    };
  ...
  async fetch(url: string): Promise<{ ok: boolean; text?: string; error?: string }> {
    ...
    try {
      const response = await globalThis.fetch(url, {
        headers: { 'User-Agent': 'DragonClaw-Research/1.0' },
