# ═══════════════════════════════════════════════════════════
# DragonClaw - Copy Files to VM Shared Folder
...
$DragonClawSource = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$ParentDir        = Split-Path $DragonClawSource
...
# Validate paths
if (-not (Test-Path $DragonClawSource)) {
    Write-Error "DragonClaw source not found at: $DragonClawSource"
    exit 1
}
...
Write-Host ""
Write-Host "  DragonClaw - Copy to VM" -ForegroundColor Cyan
Write-Host "  ======================================="
Write-Host ""

# ── Step 1: Copy DragonClaw ──
Write-Host "  [1/3] Copying DragonClaw..."
$acDest = Join-Path $SharedFolder "dragonclaw"
...
robocopy $DragonClawSource $acDest /E /NFL /NDL /NJH /NJS /NC /NS `
    /XD node_modules .git `
    /XF *.zip 2>$null
Write-Host "  OK: DragonClaw copied to shared folder" -ForegroundColor Green

# ── Step 2: Copy Author OS tools (only the parts DragonClaw integrates with) ──
...
$premiumSource = Join-Path (Split-Path $DragonClawSource) "dragonclaw-premium-bundle"
if (Test-Path $premiumSource) {
    $premDest = Join-Path $SharedFolder "dragonclaw-premium"
...
Write-Host "    cp -r /media/sf_dragonclaw-transfer/dragonclaw ~/dragonclaw"
Write-Host "    cp -r /media/sf_dragonclaw-transfer/author-os ~/author-os"
Write-Host "    cd ~/dragonclaw && npm install"
Write-Host ""
Write-Host "    # Start DragonClaw:"
Write-Host "    cd ~/dragonclaw && npx tsx gateway/src/index.ts &"
Write-Host ""
...
Write-Host "      -d '{""key"":""gemini_api_key"",""value"":""'`$(cat /media/sf_dragonclaw-transfer/key.txt)'""}'"
Write-Host ""
Write-Host "  =======================================" -ForegroundColor Cyan
Write-Host ""
