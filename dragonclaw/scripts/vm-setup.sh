...
echo "  [4/5] Creating DragonClaw directory structure..."
mkdir -p ~/dragonclaw
mkdir -p ~/author-os

echo "  Directories created:"
echo "    ~/dragonclaw        - Main application"
echo "    ~/author-os         - Author OS tools"

# ── Step 5: Set hostname ──
echo "  [5/5] Setting hostname to 'dragonclaw'..."
sudo hostnamectl set-hostname dragonclaw 2>/dev/null || true
echo "  Hostname set to 'dragonclaw'"
...
echo "     cp -r /media/sf_dragonclaw-transfer/dragonclaw ~/dragonclaw"
echo "     cp -r /media/sf_dragonclaw-transfer/author-os ~/author-os"
echo ""
echo "  2. Install deps and start DragonClaw:"
echo "     cd ~/dragonclaw && npm install"
echo "     npx tsx gateway/src/index.ts &"
echo ""
echo "  3. Or use the quick deploy script:"
echo "     bash /media/sf_dragonclaw-transfer/run.sh"
echo ""
