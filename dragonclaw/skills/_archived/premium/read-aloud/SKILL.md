...
DragonClaw uses **Piper TTS** — a fast, free, open-source text-to-speech engine:
...
### Setup (One-Time)
```bash
# Install Piper (included in DragonClaw setup wizard)
pip install piper-tts

# Download a voice model (~100MB each)
# DragonClaw will prompt you to pick one on first use
```
