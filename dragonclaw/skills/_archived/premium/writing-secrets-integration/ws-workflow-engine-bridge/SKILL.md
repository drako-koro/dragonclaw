...
description: Import and execute Author Workflow Engine prompt sequences inside DragonClaw
...
Import prompt sequence JSON files from the Author Workflow Engine and execute them as DragonClaw automations.

## What This Does

The Author Workflow Engine (AWE) lets you build multi-step prompt chains — like "research → premise → outline → write chapter 1." This bridge imports those chains and runs them inside DragonClaw with full access to your book bible, voice profile, and memory.
...
1. **Import**: Load AWE JSON into DragonClaw's automation engine
2. **Enhance**: Automatically inject DragonClaw context into each step:
...
4. **Route**: Use DragonClaw's smart AI routing per step:
   - `"model": "auto"` → Let DragonClaw pick the best provider
