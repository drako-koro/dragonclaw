# ═══════════════════════════════════════════════════════════
# DragonClaw - Prepare VirtualBox VM
...
Write-Host ""
Write-Host "  DragonClaw - VM Preparation" -ForegroundColor Cyan
Write-Host "  ======================================="
Write-Host ""
...
# ── Step 4: Update shared folder to point to DragonClaw transfer directory ──
...
try { & $VBoxManage sharedfolder remove $VMName --name "dragonclaw-transfer" 2>$null } catch {}
& $VBoxManage sharedfolder add $VMName --name "dragonclaw-transfer" --hostpath $SharedPath --automount
Write-Host "  OK: Shared folder configured at $SharedPath" -ForegroundColor Green

# ── Step 5: Add port forwarding for DragonClaw dashboard ──
Write-Host "  [5/5] Adding port forwarding (host:3847 -> guest:3847)..."
try {
    & $VBoxManage modifyvm $VMName --natpf1 delete "dragonclaw" 2>$null
} catch {}
& $VBoxManage modifyvm $VMName --natpf1 "dragonclaw,tcp,,3847,,3847"
Write-Host "  OK: Port 3847 forwarded (access dashboard from Windows)" -ForegroundColor Green

# ── Rename VM (optional - do this last) ──
Write-Host ""
Write-Host "  To rename the VM to 'DragonClaw', run:" -ForegroundColor Yellow
...
Write-Host "     cp -r /media/sf_dragonclaw-transfer/dragonclaw ~/dragonclaw" -ForegroundColor White
Write-Host "     cp -r /media/sf_dragonclaw-transfer/author-os ~/author-os" -ForegroundColor White
Write-Host "  3. Run setup: ~/dragonclaw/scripts/vm-setup.sh" -ForegroundColor White
Write-Host "  4. Log out/in, then deploy: ~/dragonclaw/scripts/deploy.sh" -ForegroundColor White
Write-Host "  =======================================" -ForegroundColor Cyan
Write-Host ""
