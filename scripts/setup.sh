#!/bin/bash
# DragonClaw Setup Wizard
# Interactive setup for first-time users

set -e

echo ""
echo "  ✍️  DragonClaw Setup Wizard"
echo "  ═══════════════════════════════════"
...
echo ""
echo "  DragonClaw supports free AND paid AI providers."
...
echo "  ═══ Security Setup ═══"
echo ""
echo "  DragonClaw encrypts all credentials with AES-256-GCM."
echo "  Set a vault passphrase (or press Enter for dev default)."
read -sp "  Vault passphrase: " VAULT_PASS
echo ""

if [ -n "$VAULT_PASS" ]; then
    echo "DRAGONCLAW_VAULT_KEY=$VAULT_PASS" > .env
