...
print_header() {
    ...
    echo -e "${PURPLE}  ╔═══════════════════════════════════════════╗${NC}"
    echo -e "${PURPLE}  ║                                           ║${NC}"
    echo -e "${PURPLE}  ║${WHITE}${BOLD}       DragonClaw Setup Wizard v4.0       ${NC}${PURPLE}║${NC}"
...
echo -e "  ${DIM}API keys are encrypted with AES-256 in DragonClaw's vault.${NC}"
echo ""
...
if [ "$PROVIDER_COUNT" -eq 0 ]; then
    fail "No AI providers configured. DragonClaw needs at least one."
...
echo -e "  DragonClaw encrypts all your API keys with AES-256."
...
if [ -f "package.json" ]; then
    ...
    fail "package.json not found. Make sure you're running this from the dragonclaw directory."
    exit 1
fi

# Store vault passphrase — export for this session AND save to .env for persistence
export DRAGONCLAW_VAULT_KEY="$VAULT_PASS"

# Write to .env so dotenv auto-loads it on every future start
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"
echo "DRAGONCLAW_VAULT_KEY=$VAULT_PASS" > "$PROJECT_DIR/.env"
...
echo ""
echo -e "  ${GREEN}${BOLD}DragonClaw is ready.${NC}"
echo ""
...
echo ""
echo -e "  ${WHITE}${BOLD}To start DragonClaw:${NC}"
echo ""
echo -e "    ${CYAN}cd ~/dragonclaw && npx tsx gateway/src/index.ts${NC}"
echo ""
...
# DragonClaw — Personalized Setup Help

Copy everything below this line and paste it into ChatGPT, Claude, Gemini, or any AI assistant to get personalized help:

---

I'm trying to set up DragonClaw, an open-source AI writing agent for authors. It's a Node.js/TypeScript project that runs directly with tsx. I need help troubleshooting my setup.

Here's what DragonClaw is:
- A Node.js 22+ server (TypeScript, uses tsx to run)
- Start command: cd ~/dragonclaw && npx tsx gateway/src/index.ts
...
ask "Start DragonClaw now? [Y/n]: "
read -r yn
if [[ ! "$yn" =~ ^[Nn]$ ]]; then
    echo ""
    echo -e "  ${WHITE}Starting DragonClaw...${NC}"
    echo ""
    cd "$(dirname "$0")/.." 2>/dev/null || cd "$(dirname "$0")"
    ...
    echo ""
    echo -e "  ${GREEN}${BOLD}DragonClaw is running!${NC}"
