# DragonClaw Launch Guide

Quick reference for starting, stopping, and managing DragonClaw.

Visual theme: obsidian black, ember orange, molten glow.
...
### Start the server
```bash
cd C:\Users\chris\OneDrive\Documents\Automations\DragonClaw\dragonclaw
npm start
```
...
```bash
taskkill /F /FI "WINDOWTITLE eq *dragonclaw*"
```
...
echo "DRAGONCLAW_VAULT_KEY=your-64-char-hex-key-here" > .env
chmod 600 .env
...
DragonClaw binds to `127.0.0.1` only — it's not exposed to the internet by default.
...
|---|---|---|
| DragonClaw | 3847 | localhost only |
